...
class Console:
	...
	def execute(self, command, value = None):
		...
		if (self.command == "pwd"):
			"""
			self.getNodes = True
			branches = self.tree.getBranches(self.getNodes)
			"""

		elif (self.command == "cd"):
			self.path = value
			self.pathParts = value.split("/")
			self.moveV = pathParts[(len(pathParts))-1]
			...
			#ls enlista todos los objetos contenidos en un directorio dado o en el actual, esto lo hace en orden alfabetico
			pass
			...
			self.valueParts = value.splt(" ")
			self.pathToMove = valueParts[0]
			self.where = valueParts[1]

			if self.tree.moveNodeTo(pathToMove, where):
				print ("%s: %s movido con exito al directorio: %s")%(command, pathToMove, where)

			else:
				printExcept(command, "InvalidMovement", where)
	...
	def exitMessage(self):
		...
		pick = random.randrange((len(messages))-1)

		print ("\n*\ %s \*\n\n")%(messages[pick])

	def sortAlphabetically(self, char, branch):
